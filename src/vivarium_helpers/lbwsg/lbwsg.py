...
def pull_lbwsg_exposure_from_gbd_2021(location_ids, year_ids=2021, save_to_hdf=None, hdf_key=None, complevel=None):
    """Calls get_draws to pull LBWSG exposure data from GBD 2019."""
    # Make sure type(location_ids) is list
    location_ids = [location_ids] if isinstance(location_ids, int) else list(location_ids)
    lbwsg_exposure = get_draws(
...
def pull_lbwsg_relative_risks_from_gbd_2021(cause_ids=None, year_ids=2021, save_to_hdf=None, hdf_key=None):
...
def get_category_to_meid_map(lbwsg_exposure_df, validate=True):
    ...
    def assert_unique_meids(series):
        assert series.map(len).unique().squeeze() == 1, \
